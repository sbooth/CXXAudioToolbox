...
class CAAUGraph final {
  public:
    ...
    /// @throw std::system_error.
    void NodeInfo(AUNode inNode, AudioComponentDescription *_Nullable outDescription,
                  AudioUnit _Nullable *_Nullable outAudioUnit) const;
    ...
    /// @throw std::system_error.
    void ConnectNodeInput(AUNode inSourceNode, UInt32 inSourceOutputNumber, AUNode inDestNode,
                          UInt32 inDestInputNumber);

    /// Sets a callback for the specified node's specified input.
    /// @throw std::system_error.
    void SetNodeInputCallback(AUNode inDestNode, UInt32 inDestInputNumber,
...
inline CAAUGraph::operator bool() const noexcept { return graph_ != nullptr; }

inline CAAUGraph::operator AUGraph const _Nullable() const noexcept { return graph_; }

inline AUGraph _Nullable CAAUGraph::get() const noexcept { return graph_; }
...
inline void CAAUGraph::swap(CAAUGraph &other) noexcept { std::swap(graph_, other.graph_); }
